...
public:
	...
	void setAttenuation(float constant, float lineal, float quadratic);			///< Set attenuation
	void setSpotExponent(float spotExp);										///< Set spot exponent
	...
	void setAngles(float pitch, float yaw);										///< Set light angles (in case of directional light
	void setIsOn(bool newIsOn);													///< Turn on/off the light
	...
	XMFLOAT3 getAttenuation();			///< Get attenuation (constant, linear, quadratic)
	...
	XMFLOAT2 getAngles();				///< Get the light angles (rads)
	bool getIsOn();						///< Get light status (true if the light is on; false if it is off)
...
protected:
	...
	float specularPower;
	XMFLOAT3 attenuation;// x = constant; y = linear; z = quadratic
	float spotExponent;
	XMVECTOR position;
	XMMATRIX viewMatrix;
